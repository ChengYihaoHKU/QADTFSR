...
def create_dataset(dataset_config):
    ...
    elif dataset_config['type'] == 'dtls':
        dataset = DtlsData(**dataset_config['params'])
    elif dataset_config['type'] == 'gen':
        dataset = GenData(**dataset_config['params']) 
...
class DtlsData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.downsize is not None:
            im_ds = cv2.resize(im_base, (self.downsize,self.downsize), interpolation=cv2.INTER_CUBIC)
            im_ds_original = self.transform(im_ds)
            # resize back
            im_ds = cv2.resize(im_ds, dsize=(im_base.shape[1], im_base.shape[0]), interpolation=cv2.INTER_CUBIC)
            im_target = self.transform(im_ds)
            
            out['lq'] = im_target
            out['lq_original'] = im_ds_original
...
class GenData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # path_hash = hashlib.md5(im_path_base.encode()).hexdigest()
        # seed = int(path_hash[:8], 16)
        # using random seed for more randomness
        seed = random.randint(0, 2**30)
        if self.downsize is not None:
            im_target = im_target.unsqueeze(0)
            im_target = F.interpolate(im_target, size=(self.downsize, self.downsize), mode='bicubic',align_corners=False)
            im_target = im_target.squeeze(0)

        if isinstance(im_target, torch.Tensor):
            generator = torch.Generator()
            generator.manual_seed(seed)
            noise = torch.randn(im_target.shape, generator=generator, dtype=im_target.dtype)
            noise = torch.clamp(noise, -1.0, 1.0) 
        else:  # numpy array
            np.random.seed(seed)
            noise = np.random.randn(*im_target.shape).astype(im_target.dtype)
